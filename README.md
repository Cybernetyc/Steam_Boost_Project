# 7_Seg (STM32F401CCU6) — 3‑разрядный 7‑сегментный индикатор + машина состояний

Основано на анализе содержимого архива `Software.zip` (проект `Software/`).
...
Если выкладываете на GitHub, обычно имеет смысл **не коммитить**:
- `cmake-build-*`, `build/`
- `.idea/` (если не хотите хранить IDE‑настройки)
- `.git/` внутри архива (в GitHub должен быть ваш собственный репозиторий)

Пример `.gitignore` (минимум):
```gitignore
/build/
/cmake-build-*/
.idea/
